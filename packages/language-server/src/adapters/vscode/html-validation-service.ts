...
export class VsCodeHtmlValidationService {
  ...
  private validateSingleNode(
    ...
    const elementDeprecation = this.checkElementDeprecation(node.tag);
    if (elementDeprecation) {
      const elementRange = this.findElementTagRange(document, node);
      if (elementRange) {
        diagnostics.push({
          severity: this.getSeverityLevel("deprecatedElement"),
          range: elementRange,
    ...
    for (const [attrName, attrValue] of Object.entries(node.attributes)) {
      ...
      const deprecation = this.checkAttributeDeprecation(node.tag, attrName);
      console.debug("DEPRECATED", attrName, deprecation);
      if (deprecation) {
        const range = this.findAttributeNameRange(document, node, attrName);
        if (range) {
          diagnostics.push({
            severity: this.getSeverityLevel("deprecatedAttribute"),
            range,
   ...
   */
  private checkElementDeprecation(tagName: string): { error: string } | null {
