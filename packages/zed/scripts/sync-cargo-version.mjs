...
if (typeof packageVersion !== "string" || packageVersion.length === 0) {
  throw new Error("Expected packages/zed/package.json to contain a valid version string");
}
...
if (!versionPattern.test(cargoToml)) {
  throw new Error("Could not find [package] version in packages/zed/Cargo.toml");
}

const updatedCargoToml = cargoToml.replace(versionPattern, `$1${packageVersion}$2`);
