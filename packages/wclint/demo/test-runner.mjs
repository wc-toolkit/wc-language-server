...
const scenarios = [
  ...
  },
  {
    ...
    command: 'node',
    args: [CLI_PATH, 'validate', '--config', 'wc.config.js', '--format', 'json', '*.html'],
    options: {},
  },
  {
    name: 'Validate with JUnit output',
    command: 'node',
    args: [CLI_PATH, 'validate', '--config', 'wc.config.js', '--format', 'junit', '*.html'],
    options: {},
  },
  {
...
for (const s of scenarios) {
  test(s.name, async () => {
    ...
    assert.ok(typeof res.code === 'number', `Process did not complete for ${s.name}`);
    assert.ok(res.code === 0 || res.code === 1, `Unexpected exit code ${res.code} for ${s.name}`);
  });
}
